/*
 Given two dimensional set of points , we have to find the nearest 
 point of or the nearest distance among the given points. 
*/
...
void findNearestNeighbour(int node, int left, int right, int x, int y) {
    ...
    if (closestDist > d && d > 0 ) {
        closestDist = d;
        closestNode = node;
        ClosestPoint = make_pair( tx[node] , ty[node] );
    }
    if (left == right) return;
...
int main() {
    ...
    For(cs,test){
        ...
        rep(i,N) {
            ...
            printf("( %d , %d ) : ( %d , %d ) = ",inp[i].ff,inp[i].ss,ClosestPoint.ff,ClosestPoint.ss);
            printf("%lld\n",closestDist);
        }
    }
    return 0;
